<?php
/**
 * Associates provider groups from the role and user group pages.
 *
 * PHP version 7.4+
 *
 * @category AddOIDCGroupTabs
 * @package  FOGProject
 ...
 * Associates provider groups from the role and user group pages.
 *
 * The Provider Group page owns the association, but "which provider groups
 * feed this?" is asked while looking at the role or the user group, and the
 * answer is only half useful if it cannot be changed there. Every other
 * association tab in FOG edits from both ends -- both ends write the same
 * table, so there is no second source of truth to drift.
 *
 * The LDAP plugin's AddLDAPGroupTabs is the same hook against the same two
 * core pages, and this deliberately follows it line for line rather than
 * inventing a second shape: an admin wiring up sign-in should meet one
 * pattern whichever directory the accounts come from. The two tabs coexist
 * on the page -- their ids differ by node -- so an install running both
 * plugins gets both.
 *
 ...
 *  - the add/remove POST is picked up from {NODE}_EDIT_SUCCESS. The core
...
class AddOIDCGroupTabs extends \FOG\Base\Hook
{
     ...
     * @var string
     */
    public $name = 'AddOIDCGroupTabs';
    /**
     * The description of this hook.
     *
     * @var string
     */
    public $description = 'Associate provider groups from a role or user group';
    /**
     * The active flag.
     *
     * @var bool
    ...
    public $node = 'oidc';
    /**
     * The node the association and its permissions belong to.
     *
     * Not $this->node: the provider group is a node of its own precisely so
     * a role can be given the ability to manage providers without the
     * ability to change what a provider group grants.
     *
     * @var string
     */
    ...
    public function injectTabData($arguments)
    {
        ...
        // The tab is a second door onto an oidcgroup association, so it
        // answers to the oidcgroup permissions and not merely to the
        ...
        $arguments['pluginsTabData'][] = [
            'name' => _('Provider Groups'),
            'id' => $node . '-' . self::GROUP_NODE,
     ...
     * page from the role page would bounce the browser to the provider
     * group list whenever no provider group happened to share the role's
     * id.
     *
    ...
    public function renderTab($obj, $node)
    {
        ...
        echo _('Provider Group Associations');
        echo '</h4>';
        echo '<p class="form-text">';
        echo(
            $isRole ?
            _(
                'Anyone signing in through one of these provider groups '
     ...
     *
     * The association is written through the OIDCGroup entity rather than
     * by batch-inserting rows, so this shares the deduplication and the
     * cascade the Provider Group page already goes through. The owner is
     * the role or user group, so the loop is over the selected groups.
     *
     * @param mixed $arguments The arguments to change.
     *
     * @return void
    ...
    public function editSuccess($arguments)
    {
        ...
        if (!\FOG\Auth\Authorization::can(self::GROUP_NODE . '.edit')) {
            ...
            $arguments['msg'] = json_encode(
                [
                    'error' => _(
                        'You do not have permission to change provider group '
                        . 'associations.'
                    ),
                    'title' => _('Provider Group Update Fail')
                ]
            );
            return;
        }
        ...
        $arguments['msg'] = json_encode(
            [
                'msg' => _('Provider Group associations updated!'),
                'title' => _('Provider Group Update Success')
            ]
        );
    }
}
