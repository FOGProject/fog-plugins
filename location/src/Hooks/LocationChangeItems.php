 ...
 */
/**
 * Named for its plugin, like every other hook here. That used to be
 * load-bearing: the class name was derived from the filename, and
 * startClassFromFiles() skipped any name already declared. This and the OU
 * plugin's hook were both changeitems.hook.php, so whichever the autoloader's
 * file walk reached first -- readdir order -- was the only one that ever
 * loaded, and the other plugin's registrations silently never happened. It
 * did not even need both plugins installed: registerInstalled() bails when
 * $node is not an installed plugin, so an uninstalled location plugin winning
 * the walk left the OU plugin's AD hook unregistered with nothing to show for
 * it.
 *
 * Two same-named hooks could no longer collide -- each plugin has its own
 * namespace, so each derives its own path (fogproject ADR 0035) -- but the
 * distinct names stay. They are what the log and the hook list show, and
 * "ChangeItems" twice would be unreadable whether or not it still broke.
 */
